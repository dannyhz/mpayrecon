 ...
 */
@Component
public class TongLianChannelBill extends AbstractChannelBill {
        ...
        try {
            if (fileList != null && fileList.size() > 0) {
                for (byte[] bill : fileList) {
                    ...
                    String _tonglian_status = billStr[2];// 状态
                    logger.info("文件数据:流水号{},金额{},状态{}", _trx_id, _file_amount, _tonglian_status);

                    // 添加文件数据进错账记录
                    billDetailDO.setFile_trx_id(_trx_id);
                    billDetailDO.setFile_amount(_file_amount);
                    billDetailDO.setChannelOrderStatus(_tonglian_status);
                    ...
                    if (dbMap.containsKey(keyObj)) {
                        ...
                        // 比对状态,认为状态成功:通联状态成功0000或4000,订单状态00-成功，30-未知
                        if ((("00".equals(_status)
                            && (("0000").equals(_tonglian_status) || ("4000").equals(_tonglian_status)))
                            || ("30".equals(_status))
                                && (("0000").equals(_tonglian_status) || ("4000").equals(_tonglian_status)))) {
                            // 比对金额，相同，则认为成功
    ...
    @Override
    protected List<byte[]> doParseChannelLocalFile(InputStream inputStream) throws Exception {
        try {
    ...
    public byte[] handleLine(String unHandleStr) throws Exception {
        ...
        try {
            String _trx_id = StringUtils.fatch(unHandleStr, 0, " ");
            if (!StringUtils.isEmpty(trxIdFlag) && trxIdFlag.equals(_trx_id.substring(0, 1))
                && (!"PDSMK".equals(_trx_id))) {// 排除汇总这行
                resStr = (_trx_id + // 流水号trx_id
                    "," + StringUtils.fatch(unHandleStr, 4, " ") + "," + StringUtils.fatch(unHandleStr, 3, " "))
                        .getBytes();// 金额
            }
        } catch (Exception e) {
    ...
    protected void doinit() {
        super.setChannelName("通联支付");
        super.setChannelCode(PropertyUtils.getValue("com.sunrun.bill.tl.channelCode"));
        super.setControlFlag(PropertyUtils.getValue("com.sunrun.bill.tl.controlFlag"));
        super.setLocalfilepath(PropertyUtils.getValue("com.sunrun.bill.bank.local.tlfilepath"));
        super.setFileName(PropertyUtils.getValue("com.sunrun.bill.tl.fileName"));
        super.setSftpPath(PropertyUtils.getValue("com.sunrun.bill.sftp.tl.filepath"));
    }
