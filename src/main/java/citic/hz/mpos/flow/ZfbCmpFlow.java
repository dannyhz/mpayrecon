 ...
 *
 */
@WebServlet("/zfbCmpFlow")
public class ZfbCmpFlow extends AbstractApiSvlt{
	...
	public JSONObject cmpStd(HttpServletRequest request,HttpServletResponse response, JSONObject reqJson) throws ServletException, IOException {
		...
		if("0".equals(Globle.config.get("zfbCmpFlow"))){
			log.warn("支付宝对账配置为暂停");
			return rj.append("_respcd", "-101").append("_respMsg", "支付宝对账配置为暂停");
		}	
		...
		//一天只做一次
		JSONObject rt = CheckLimitService.checkLimit("MPOS_BAT_ZFBCMP", trdt, 1);	
		if(rt.getInt("respcd") != 0)
			return rj.append("_respcd", "-100").append("_respMsg", "该日对账不得重复");
		
		if(Env.needLoadData){
			
			CmpGetDtlService.getZfbFiles(fTrdt);
...
//			}
		...
		WxNotifyService.offerBizMsg("支付宝商户清分计算完毕");
		...
		WxNotifyService.offerMsg("支付宝交易分润计算完成");
		...
		WxNotifyService.offerMsg("支付宝对账结果回写完成");
